...
4.  **Deploy**: Click Create. Copy the resulting URL (e.g., `https://lumina-backend.onrender.com`).
...
3.  **Environment Variables**:
    *   `VITE_API_URL`: Your Render Backend URL (e.g., `https://lumina-backend.onrender.com`).
4.  **Deploy**: Click Deploy.

## 3. Final Connection
1.  Copy your new Vercel domain (e.g., `https://lumina.vercel.app`).
2.  Go back to Render -> Environment Variables.
3.  Add `FRONTEND_URL` = `https://lumina.vercel.app`.
...
This guide walks through deploying the LesionRec AI skin analysis system to Google Cloud Platform (GCP).
...
export PROJECT_ID="lesionrec-prod"

# Create new project
gcloud projects create $PROJECT_ID --name="Lumina AI"
...
gcloud iam service-accounts create lesionrec-backend \
    --display-name="LesionRec Backend Service Account"

# Grant necessary permissions
export SERVICE_ACCOUNT="lesionrec-backend@${PROJECT_ID}.iam.gserviceaccount.com"
...
PROJECT_ID = os.getenv("GCP_PROJECT_ID", "lesionrec-prod")
...
```bash
# Google Cloud
GCP_PROJECT_ID=lesionrec-prod
GCP_REGION=us-central1
GCS_BUCKET_NAME=lesionrec-prod-uploads
...
# Build Docker image
docker build -t lesionrec-backend .
...
export SERVICE_NAME="lesionrec-api"
...
steps:
  ...
  - name: 'gcr.io/cloud-builders/docker'
    args: ['build', '-t', 'gcr.io/$PROJECT_ID/lesionrec-backend:$COMMIT_SHA', './backend']

  # Push the container image to Container Registry
  - name: 'gcr.io/cloud-builders/docker'
    args: ['push', 'gcr.io/$PROJECT_ID/lesionrec-backend:$COMMIT_SHA']
  ...
  - name: 'gcr.io/google.com/cloudsdktool/cloud-sdk'
    ...
    args:
      ...
      - 'lesionrec-api'
      - '--image'
      - 'gcr.io/$PROJECT_ID/lesionrec-backend:$COMMIT_SHA'
      - '--region'
      - 'us-central1'
      - '--platform'
      - 'managed'
...
```bash
VITE_API_URL=https://lesionrec-api-xxxxx-uc.a.run.app
```
...
# Navigate to: Cloud Run > lesionrec-api > Metrics
```
